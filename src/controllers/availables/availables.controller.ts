...
export class AvailablesController {
  constructor(private readonly availablesService: AvailablesService) {}
  ...
  async create(@Body() available: CreateAvailableDto): Promise<Available> {
    ...
    if (isCreated) {
      return await this.availablesService.findOne(isCreated.id)
    } else {
  ...
  async update(
    @Param('id', new ParseIntPipe())
    id,
    @Body() available: UpdateAvailableDto
  ) {
    try {
