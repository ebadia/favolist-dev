import { Controller, Get, Post, Req, Param, Body, Query, ForbiddenException, BadRequestException, Patch, Delete } from '@nestjs/common'
...
export class ShopsController {
  ...
  @Get(':id/users')
  async findOneWithUsers(@Param('id', new ParseIntPipe()) id) {
    // console.log(id)
     return await this.shopsService.findOneWithUsers(id);
  }

  @Get(':id/products')
  async findOneWithProducts(@Param('id', new ParseIntPipe()) id) {
    // console.log(id)
     return await this.shopsService.findOneWithProducts(id);
  }

  @Get(':id/admins')
  async findOneWithAdmins(@Param('id', new ParseIntPipe()) id) {
    // console.log(id)
     return await this.shopsService.findOneWithAdmins(id);
  }

  @Get(':id/orders')
  async findOneWithOrders(@Param('id', new ParseIntPipe()) id, @Query('day') day ) {
    // console.log(id)
  ...
  @Patch(':id')
  @UsePipes(new ValidationPipe())
  async update( @Param('id', new ParseIntPipe()) id, @Body() shop: UpdateShopDto ) {
    try {
  ...
  @Post(':id/products')
  // @UsePipes(new ValidationPipe())
  async addProductToShop( @Param('id', new ParseIntPipe()) id, @Body() product: AssignShopDto ) {
    // throw new ForbiddenException()
    try {
  ...
  @Post(':id/admins')
  // @UsePipes(new ValidationPipe())
  async addAdminToShop( @Param('id', new ParseIntPipe()) id, @Body() admin: CreateAdminDto ) {
    // throw new ForbiddenException()
    try {
