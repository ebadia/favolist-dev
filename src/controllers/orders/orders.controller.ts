...
export class OrdersController {
  ...
  @Get(':id')
  async find( @Param('id', new ParseIntPipe()) id ): Promise<Order> {
  ...
  @Post(':id/items')
  @UsePipes(new ValidationPipe())
  async addItemToOrder( @Param('id', new ParseIntPipe()) id, @Body() item: CreateItemDto ): Promise<Order> {
    try {
      // save item first
  ...
  @Patch(':id')
  @UsePipes(new ValidationPipe())
  async update( @Param('id', new ParseIntPipe()) id, @Body() order: UpdateOrderDto ): Promise<Order> {
    try {
  ...
  @Get('/today/shops/:id')
  async findTodayFromShop( @Param('id', new ParseIntPipe()) id ): Promise<Order[]> {
    const date = new Date(Date.now()).toISOString().split('T')[0]
    return await this.ordersService.findFromShop( id, date )
  }

  @Get('/today/users/:id')
  async findTodayFromUser( @Param('id', new ParseIntPipe()) id ): Promise<Order[]> {
    const date = new Date(Date.now()).toISOString().split('T')[0]
    return await this.ordersService.findFromUser( id, date )
  }

  @Get('/today/users/:userId/shops/:shopId')
  async findTodayFromUserShop( @Param('userId', new ParseIntPipe()) userId, @Param('shopId', new ParseIntPipe()) shopId ): Promise<Order[]> {
    const date = new Date(Date.now()).toISOString().split('T')[0]
    return await this.ordersService.findFromUserShop( userId, shopId, date )
  }

  @Get('/date/shops/:id')
  async findDateFromShop( @Param('id', new ParseIntPipe()) id, @Query('day') date ): Promise<Order[]> {
    return await this.ordersService.findFromShop( id, date )
  }
